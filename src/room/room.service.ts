...
export class RoomService {
  ...
  async addRoom(roomName: Room['name'], hostId: User['userId']): Promise<void> {
    const hostUser = await this.userService.getStudentById(hostId);
  ...
  async getRoomHost(hostName: Room['host']['userName']): Promise<User> {
    ...
    return new Student(this.rooms[roomIndex].host);
  }
  ...
  async addUserToRoom(
    ...
    const roomIndex = await this.getRoomIndexByName(roomName);
    const newUser = await this.userService.getStudentById(userId);
  ...
  async getFirstInstanceOfUser(
    ...
    return new Student(findUserInRoom);
  }
